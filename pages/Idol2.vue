  ...
  <div class="app" @touchstart="onAppTouchStart" @touchend="onAppTouchEnd">
    ...
    <header class="bar">
      <div class="bar-left">
        ...
        <!-- Artist avatar + name + progress in bar for works/detail -->
        ...
        <div v-if="currentView === 'detail' && currentArtist" class="bar-artist">
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
            style="color: var(--ink3); flex-shrink:0">
      ...
      <div class="bar-right">
        <div class="search-box">
          ...
          <input ref="searchInput" v-model="searchQuery" type="text"
            :placeholder="currentView === 'works' ? 'Filter works...' : 'Search...'" @input="handleSearch" />
        ...
        <button v-if="currentView === 'works' || currentView === 'artists'"
          @click="viewFilter = viewFilter === 'all' ? 'unviewed' : viewFilter === 'unviewed' ? 'viewed' : 'all'"
      ...
      <main v-if="currentView === 'artists'" key="artists" class="page">
        ...
        <div v-if="alphabeticalGroups.length > 3" class="alpha-rail">
          <button v-for="letter in allLetters" :key="letter" @click="scrollToLetter(letter)"
            :class="{ active: alphabeticalGroups.indexOf(letter) !== -1 }"
            :disabled="alphabeticalGroups.indexOf(letter) === -1">{{ letter }}</button>
        </div>
        ...
        <div v-for="letter in alphabeticalGroups" :key="letter" :ref="'group-' + letter" class="group">
          ...
          <div class="grid-artists">
            <div v-for="artist in groupedArtists[letter]" :key="artist.name" class="a-card"
              :class="{ dim: viewedArtists.indexOf(artist.name) !== -1 && viewFilter === 'all' }"
      ...
      <main v-else-if="currentView === 'works'" key="works" class="page">
        <div class="works-top">
          <div class="works-info">
            <div class="chips">
              <span v-if="currentArtist && currentArtist.mainWorks && currentArtist.mainWorks.length" class="chip">{{
    currentArtist.mainWorks.length }} main</span>
              <span v-if="currentArtist && currentArtist.compilations && currentArtist.compilations.length"
                class="chip">{{ currentArtist.compilations.length }} comp</span>
          ...
          <div v-if="section.items.length" class="w-section">
            ...
            <div class="grid-works">
              <div v-for="work in section.items" :key="work.code" class="w-card"
                :class="{ dim: viewedWorks.indexOf(work.code) !== -1 && viewFilter === 'all' }"
      ...
      <div v-if="showUploadModal" class="overlay" @click.self="closeUploadModal">
        <div class="modal">
          ...
          <div class="m-foot">
            ...
            <button @click="handleCustomImageUrl" class="btn-fill">{{ customImageUrl.trim() ? 'Save' : 'Remove'
              }}</button>
          </div>
        </div>
      </div>
    ...
    <transition name="fade">
      <div v-if="lightbox.show" class="lb" @click.self="closeLightbox">
        ...
        <button v-if="lightbox.images.length > 1" class="lb-arr left" @click="prevImage"
          :disabled="lightbox.currentIndex === 0">&lt;</button>
        <div class="lb-main"><img :src="lightbox.images[lightbox.currentIndex]" :alt="lightbox.code"
            @error="onImgError" /></div>
        <button v-if="lightbox.images.length > 1" class="lb-arr right" @click="nextImage"
...
//  External links constant - moved out of data()
var EXTERNAL_LINKS = [
  { key: 'njav', label: 'NJAV' },
...
export default {
  ...
  data: function () {
    return {
      ...
      showAddWorkModal: false, showUploadModal: false,
      ...
      appTouchStartX: 0, appTouchStartY: 0,
  ...
  methods: {
    ...
    setupKeyboardShortcuts: function () {
      ...
      this._keydownHandler = function (e) {
        ...
        //  NEW: '/' focuses search when not already in an input
        if (e.key === '/' && !isInput) {
          e.preventDefault()
      ...
      //  NEW: Show shortcut hint on first visit to artists view after a short delay
      setTimeout(function () {
        if (self.currentView === 'artists') {
          self.showShortcutHint = true
          setTimeout(function () { self.showShortcutHint = false }, 3000)
        }
      }, 1500)
    },
    ...
    onGalleryImgError: function (e, index) {
      ...
      // Also hide the skeleton inside the parent thumb
    ...
    getArtistViewedCount: function (artist) {
      ...
      var self = this
      return all.filter(function (w) { return self.viewedWorks.indexOf(w.code) !== -1 }).length
    },
    ...
    onAppTouchEnd: function (e) {
      // Only handle on works view, not detail (detail has its own swipe)
      ...
      // Swipe right with clear horizontal intent (dx > 80 to avoid accidental triggers)
    ...
    exportData: function () {
      try {
        ...
        var arr = this.artists.map(function (a) {
          return { name: a.name, cover: a.cover || '', compilations: (a.compilations || []).map(function (w) { return w.code }).sort(), mainWorks: (a.mainWorks || []).map(function (w) { return w.code }).sort() }
    ...
    hardRefresh: function () {
      ...
      var self = this
        ;['artists', 'viewedArtists', 'viewedWorks', 'artistSortBy', 'customImages'].forEach(function (k) { localStorage.removeItem(k) })
      ...
      p.then(function () {
        ...
        self.currentView = 'artists'; self.activeTab = ''; self.customImages = {}
...
/*  NEW: dimmed artist name in detail breadcrumb */
.bar-title-dim {
  color: var(--ink3);
  max-width: 110px;
  font-size: 13px;
...
/* Actions toolbar: two groups separated by a divider */
.actions {
  display: flex;
  align-items: center;
  gap: 0;
...
.act-group+.act-group {
  border-left: 1.5px solid var(--line2);
}
...
/* Copy code button — slightly distinct */
.act-copy {
  color: var(--ink);
  font-weight: 700;
}
...
/* Unified section card for links, gallery */
.detail-section {
  background: var(--surface);
  border: 1.5px solid var(--line2);
  border-radius: var(--r);
...
@media (max-width: 640px) {
  ...
  /* Hide the chevron in detail bar on small screens too */
  .bar-artist svg {
    display: none;
  }
